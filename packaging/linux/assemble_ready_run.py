#!/usr/bin/env python3
"""Assemble one checksum-bound, deterministic Linux ready-run archive."""
...
def assemble(args: argparse.Namespace) -> dict[str, Any]:
    ...
    verification.mkdir()
    build_identity = {
        "schema": "kazstem-linux-build-identity-v2",
    ...
    (verification / "PLATFORM-TARGET.json").write_bytes(
        json_bytes(
            {
                "schema": "kazstem-linux-platform-target-v2",
                **identity["platform"],
            }
        )
    )
    ...
    ledger = {
        "schema": "kazstem-linux-module-native-inclusion-ledger-v2",
        "release": identity["release"],
        "source_commit": identity["source_commit"],
