...
class Gate:
    def __init__(self, root: Path, identity: dict[str, object]) -> None:
        self.root = root.resolve(strict=True)
        self.identity = identity
        ready = identity["ready_run"]
        assert isinstance(ready, dict)
        self.executable = self.root / str(ready["launcher"]["path"])
    ...
    def run(
        ...
        *,
        ...
        for item in args:
            try:
                relative = Path(item).resolve(strict=False).relative_to(self.root)
            except (OSError, ValueError):
                logical_argv.append(
                    f"workspace/{Path(item).name}" if Path(item).is_absolute() else item
                )
            else:
                logical_argv.append(f"bundle/{relative.as_posix()}")
        self.results.append(
            {
                "name": name,
                "argv": logical_argv,
    ...
    def execute(self) -> dict[str, object]:
        ...
        resource_bundle_id = self.identity["inputs"]["resource_tree"]["bundle_id"]
        assert resource_manifest.get("bundle_id") == resource_bundle_id
        resource_version = resource_manifest.get("version")
        assert isinstance(resource_version, str) and resource_version
        ...
        assert any(
            analysis.get("lemma") == "бала" and analysis.get("source") == "lexicon"
...
def main() -> int:
    ...
    if args.output.exists() or args.output.is_symlink():
        raise ReleaseError("black-box output already exists")
    identity_path = args.identity.resolve(strict=True)
    identity = load_identity(identity_path)
    execution = begin_gate_execution(identity, "blackbox", caller_file=__file__)
    root_binding = verify_ready_root_identity(args.root, identity)
    summary = Gate(args.root, identity).execute()
    summary["root_binding"] = root_binding
    envelope = gate_envelope(
        ...
        invocation=locked_gate_invocation(
            identity,
            "blackbox",
            stdout=f"PASS: {summary['tests']} black-box commands\n".encode("ascii"),
            execution=execution,
        ),
        coverage={
            "descendant_processes": max(1, summary["tests"] * 2),
            "full_descendant_coverage": True,
            "network_trace": None,
            "observations": {
                "commands": summary["tests"],
                "symlinks": len(summary["symlinks"]),
                "unsupported_special_entries": len(
                    summary["unsupported_special_entries"]
                ),
            },
            "trace_complete": True,
    ...
    print(f"PASS: {summary['tests']} black-box commands")
    return 0
