#!/usr/bin/env python3
"""Safely extract and completely audit one macOS ready-run archive."""
...
def audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    execution = begin_gate_execution(
        identity, "ready-archive-audit", caller_file=__file__
    )
    ...
    if (
        manifest["schema"] != "kazstem-macos-ready-run-manifest-v2"
    ...
    if executable_paths != sorted(set(executable_paths)):
        raise ReleaseError("ready-run executable paths must be sorted and unique")
    ...
    try:
        verify_declared_archive_inventory(root, {})
    except ReleaseError as exc:
        raise ReleaseError(
            f"source/build archives are forbidden in ready-run: {exc}"
        ) from exc
    ...
    envelope = gate_envelope(
        ...
        invocation=locked_gate_invocation(
            identity,
            "ready-archive-audit",
            stdout=b"PASS: ready-run archive audited\n",
            execution=execution,
        ),
        coverage={
...
def main() -> int:
    ...
    audit(args)
    print("PASS: ready-run archive audited")
    return 0
