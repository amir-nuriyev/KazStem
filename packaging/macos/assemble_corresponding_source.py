...
def assemble(args: argparse.Namespace) -> dict[str, Any]:
    ...
    manifest = {
        "schema": "kazstem-macos-corresponding-source-manifest-v3",
    ...
    if getattr(args, "canonical_tar_only", False):
        return {
            "schema": "kazstem-macos-source-assembly-receipt-v1",
    ...
    compression_receipt = compress_canonical_tar(
        canonical_tar,
        output,
        policy=identity["compression"]["corresponding_source"],
    )
    verify_or_observe_output(
        output,
...
def main() -> int:
    ...
    if args.receipt.exists() or args.receipt.is_symlink():
        raise ReleaseError("source assembly receipt already exists")
