#!/usr/bin/env python3
"""Safely extract and completely audit the macOS source companion."""
...
from release_common import (
    ...
    fresh_extract_xattr_observation,
    gate_envelope,
    identity_sha256,
    inspect_nested,
    inspect_tar,
...
def _verify_payload_inventory(
    ...
    observed_paths = {
        item["path"]
        for item in tree_inventory(root, allow_os_provenance=True)
...
def _verify_embedded_git_tree(
    ...
    with tarfile.open(archive_path, "r:*") as archive:
        ...
        for member in members:
            ...
            if stat.S_IMODE(metadata.st_mode) != expected_mode:
                raise ReleaseError(f"embedded Git tree mode mismatch: {logical}")
            if member.kind == "file":
    ...
    observed_paths = {
        f"tree/{item['path']}"
        for item in tree_inventory(application / "tree", allow_os_provenance=True)
    } | {"tree"}
...
def audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    execution = begin_gate_execution(
        identity, "source-archive-audit", caller_file=__file__
    )
    source = identity["corresponding_source"]
    ...
    xattrs = fresh_extract_xattr_observation(root)
    ...
    if (
        manifest["schema"] != "kazstem-macos-corresponding-source-manifest-v3"
    ...
    if manifest["git_source_materialization"] != read_json(
        root / source["source_categories"]["application_source"] / "GIT-SOURCE.json"
    ):
    ...
    envelope = gate_envelope(
        ...
        invocation=locked_gate_invocation(
            ...
            stdout=b"PASS: corresponding-source archive audited\n",
            execution=execution,
        ),
        coverage={
            ...
            "observations": {
                "members": len(members),
                "nested_archives": len(nested_results),
                "nested_expanded_bytes": total_expanded,
                "provenance_xattrs": xattrs["count"],
            },
            "trace_complete": True,
...
def main() -> int:
    ...
    audit(args)
    print("PASS: corresponding-source archive audited")
    return 0
