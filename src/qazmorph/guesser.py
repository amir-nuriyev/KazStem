...
def _depthwise_lemma_interleave(
    ...
    every root present in the former top-k remains present in the new top-k:
    its first occurrence had at most k distinct roots at or ahead of it.
    """
...
def productive_surface_eligible(surface: str) -> bool:
    ...
    Analysis and productive generation share this exact public prefilter, so a
    generated surface cannot bypass a rejection that precedes analyzer lookup.
    """
...
def productive_root_kind(
    ...
    pairs because that would hallucinate roots such as ``каталок``.
    """
